# A5 WAVE9 — EVENTOS production verification

**Agent:** A5-wave9-events  
...
Wave 9 is **production verification only** — no new events, no copy rewrites, no spine/flag/weight surgery.
...
| Code changes this wave | **none** (WAVE7–8 already zeroed residuals) |
...
Grep (case-insensitive, word-aware where applicable) across entire `src/**` for:
...
Cross-check script: every preferred id on location configs matched against authored event `id` fields in:
...
| Path | Role |
|------|------|
| `.agents/swarm-grok/reports/A5-wave9-events.md` | this report only |

No source edits — residual surface and resolve map already clean after WAVE7–8.
