import React from 'react';
import './exploration.css';

interface DangerLevelBarProps {
  level: number | null;
  showLabel?: boolean;
}

const DangerLevelBar: React.FC<DangerLevelBarProps> = ({ level, showLabel = true }) => {
  const segments = [1, 2, 3, 4, 5, 6, 7];

  // Get segment class based on level
  const getSegmentClass = (seg: number): string => {
    if (level === null || seg > level) {
      return 'level-bar__segment level-bar__segment--danger-empty';
    }
    return `level-bar__segment level-bar__segment--danger-${seg}`;
  };

  // Get value class for text color
  const getValueClass = (): string => {
    if (level === null) return 'level-bar__value';
    return `level-bar__value level-bar__value--danger-${level}`;
  };

  return (
    <div className="level-bar">
      {showLabel && (
        <span className="level-bar__label">
          {level !== null ? 'Danger' : '???'}
        </span>
      )}
      <div className="level-bar__segments">
        {segments.map((seg) => (
          <div key={seg} className={getSegmentClass(seg)} />
        ))}
      </div>
      <span className={getValueClass()}>{level !== null ? level : '?'}</span>
    </div>
  );
};

export default DangerLevelBar;
